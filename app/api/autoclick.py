...
class AutoClickUpdate(BaseModel):
    ...
    interval_seconds: int = Field(default=10, ge=MIN_INTERVAL_SECONDS, le=MAX_INTERVAL_SECONDS)

    @field_validator("selected_action")
    @classmethod
    def validate_action(cls, value: str) -> str:
        if value not in ALLOWED_ACTIONS:
            raise ValueError("Invalid AutoClick action")
        return value
...
def serialize_autoclick(setting: AutoClickSetting | None) -> dict:
    ...
    return {
        ...
        "interval_seconds": int(setting.interval_seconds or 10),
